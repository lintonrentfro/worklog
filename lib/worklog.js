wl = {
    ...
    daily_log_csv_report : function(log) {
        // this function takes a daily log obj
        // and returns a csv string designed to
        // contain a snapshot of that day's log
        // in a tabular format
        var csv = "";
        csv += "Day,Work Item,Task,Completed Time,Completed By\n";
        for(var i=0;log.work_items.length>i;i++) {
            for(var ii=0;log.work_items[i].tasks.length>ii;ii++) {
                if(log.work_items[i].tasks[ii].completed_time) {
                    var hours = log.work_items[i].tasks[ii].completed_time.getHours();
                    var minutes = log.work_items[i].tasks[ii].completed_time.getMinutes();
                    var time = hours + ":" + minutes;
                };
                if(log.work_items[i].tasks[ii].completed_by) {
                    var user = log.work_items[i].tasks[ii].completed_by;
                };
                csv += log.day + "," + log.work_items[i].name + "," + log.work_items[i].tasks[ii].name + ",";
                if(time) {
                    csv += time + ",";
                } else {
                    csv += ",";
